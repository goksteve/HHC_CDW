...
col index_name              format a20                  truncate
...
select lpad(case when num_rows < 1e5 then num_rows || ' '
     ...
     , (select sum(bytes)/1024/1024/1024 from user_segments where segment_name = table_name) seg_size
     ...
     , partitioned
     , iot_type
  from user_tables t where table_name = :tab_name;
